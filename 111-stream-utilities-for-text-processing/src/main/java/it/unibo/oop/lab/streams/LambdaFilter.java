...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
        ...
        // 2)
        NUMCHAR("Count the number of chars", s -> String.valueOf(s.length())),
        // 3)
        NUMLINE("Count the number of lines", s -> String.valueOf(s.lines().count())),
        // 4)
        LISTWORD("List all the words in alphabetical order", 
        s -> Arrays.stream(s.split("\\s+"))
            .sorted()
            .collect(Collectors.joining("\n"))
        ),
        // 5)
        WORDCOUNT("Write the count for each word",
        s -> Arrays.stream(s.split("\\s+"))
